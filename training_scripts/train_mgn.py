...
parser.add_argument(
    "--data_dir", type=str, default="/mnt/d/data/cylinder_flow"
)
args = parser.parse_args()

wandb.init(
    project="train_mgn",
    entity="master-thesis",
    name="test_run_deepmind_dataset",
    # config=config,
    save_code=True,
)
...
#         default="../configfiles/config_cylinderflow_np.ini",
#     )
#     return parser
...
DS = getattr(
    import_module("GNN.DatasetClasses." + ds_class_name), ds_class_name
)
train_dataset = DS(**train_data_params, dataset_dir=args.data_dir)
test_dataset = DS(**test_data_params, dataset_dir=args.data_dir)
...
if scheduler == "ExpLR":
    ...
    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, patience=50, min_lr=1e-8
    )
else:
...
if do_rollout_test:
    ...
    update_function = (
        None
        if not hasattr(train_dataset, "update_function")
        else train_dataset.update_function
    )
