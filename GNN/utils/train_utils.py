    ...
    for i, data_list in enumerate(tqdm_itr):
        ...
        if (tb_writer is not None) and (total_steps % tb_rate == 0):
            tb_writer.add_scalar(
                "Loss/train",
                scalar_value=running_loss / tb_rate,
...
def val_epoch(
    ...
    for i, data_list in enumerate(tqdm_itr):
        ...
        if tb_writer is not None:
            tb_writer.add_scalar(
                "Loss/val", scalar_value=val_loss, global_step=total_steps
            )
            tb_writer.add_scalar(
...
def get_free_gpus(mem_threshold=100):
    ...
    free_gpus = [
        i for i, mem in enumerate(memory_available) if mem <= mem_threshold
    ]
...
def load_model(
    ...
    if exists(join(checkpoint_dir, scheduler_filename)):
        scheduler.load_state_dict(
            load(join(checkpoint_dir, scheduler_filename))
        )
