...
class ModelConfig:
    def __init__(self, config_file):
        ...
        self.train_dir = (
            None if "train_dir" not in c_data else c_data["train_dir"]
        )
        self.test_dir = (
            None if "test_dir" not in c_data else c_data["test_dir"]
        )
        ...
        self.radius = (
            0.01 if "radius" not in c_data else c_data.getfloat("radius")
        )
        ...
        self.window_length = (
            5
            if "window_length" not in c_data
            else c_data.getint("window_length")
        )
        self.apply_onehot = (
            False
            if "apply_onehot" not in c_data
            else c_data.getboolean("apply_onehot")
        )
        ...
        self.noise = (
            None if "noise" not in c_data else literal_eval(c_data["noise"])
        )
        self.noise_gamma = (
            0.1
            if "noise_gamma" not in c_data
            else c_data.getfloat("noise_gamma")
        )
        self.normalize = (
            True
            if "normalize" not in c_data
            else c_data.getboolean("normalize")
        )
        ...
        self.mgn_dim = (
            128 if "mgn_dim" not in m_data else m_data.getint("mgn_dim")
        )
        self.mp_iterations = (
            15
            if "mp_iterations" not in m_data
            else m_data.getint("mp_iterations")
        )
        self.mlp_norm_type = (
            "LayerNorm"
            if "mlp_norm_type" not in m_data
            else m_data["mlp_norm_type"]
        )
        self.mlp_norm_type = (
        ...
        self.epochs = (
            100 if "epochs" not in t_data else t_data.getint("epochs")
        )
        self.batch_size = (
            8 if "batch_size" not in t_data else t_data.getint("batch_size")
        )
        self.scheduler = (
            "ExpLR" if "scheduler" not in t_data else t_data["scheduler"]
        )
        self.use_parallel = (
            False
            if "use_parallel" not in t_data
            else t_data.getboolean("use_parallel")
        )
        ...
        self.tb_rate = (
            10 if "tb_rate" not in t_data else t_data.getint("tb_rate")
        )
        self.log_rate = (
            100 if "log_rate" not in t_data else t_data.getint("log_rate")
        )
